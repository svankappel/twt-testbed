...
def extract_test_setup(json_file_path):
    ...
    test_title = data.get("test_results", {}).get("test_title", "No Title")

    testbed_setup = data.get("test_results", {}).get("testbed_setup", {})
    testbed_settings = []
    ...
    test_results = data.get("test_results", {}).get("results", {})
    results = []
    ...
    text.textLines("\n".join(results))
    c.drawText(text)
    yPos -= 20 + (len(results)*12)

    if latency_histogram:
        #latency histogram
        c.setFont("Helvetica-Bold", 14)
        c.drawString(xPos, yPos, "Latency Histogram")
        yPos -= 310

        # Add histogram to PDF at the bottom
        c.drawImage(tmpfile_path, (width-500)/2, yPos, width=500, height=300)
